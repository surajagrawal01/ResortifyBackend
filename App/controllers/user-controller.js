...
userCntrl.forgotPassword = async (req, res) => {
  ...
  const { email, otp, password } = _.pick(req.body, [
    "email",
    "otp",
    "password",
  ]);
...
userCntrl.updatePassword = async (req, res) => {
  ...
  const { password, newPassword } = _.pick(req.body, [
    "password",
    "newPassword",
  ]);
  try {
  ...
  try {
    const user = await User.findById(req.user.id)
      ...
      .populate({
        path: "myBookings",
        populate: {
          path: "propertyId", // Specify the field you want to populate
          model: "Property", // Assuming Property is the name of the model
        },
...
userCntrl.login = async (req, res) => {
  ...
  try {
    ...
    if (!user.isVerified) {
      return res
        .status(400)
        .json({ error: "Email not verified.. Click here to verify" });
    }
    const checkPassword = await bcryptjs.compare(password, user.password);
    if (!checkPassword) {
...
userCntrl.recentSearches = (req, res) => {
